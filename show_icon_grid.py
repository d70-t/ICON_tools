...
def show_icon_grid(ds, args):
    ...
    for i, eidx in enumerate(ds.edge_vertices.data.T):
        ...
        if args.el:
            text = "{:.1f}".format(ds.edge_length.data[i])
        else:
            text = "{:d}".format(i)
        plt.text(center[0], center[1], text, horizontalalignment="center", verticalalignment="center", color="red")
...
def _main():
    ...
    parser.add_argument("--el", help="show edge_length", default=False, action="store_true")

    args = parser.parse_args()
    ds = xr.open_dataset(args.gridfile)
    show_icon_grid(ds, args)
